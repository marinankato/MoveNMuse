...
export default function SessionList({
  ...
  if (!sessions.length)
    return <div className="text-gray-500 text-sm">No upcoming sessions</div>;
  ...
  return (
    <ul className="space-y-3">
      {sessions.map((s) => {
        ...
        // construct hint message
        const hint = roleBlocksPurchase
          ...
          >
            <div>
              ...
              <div className="text-xs text-gray-500 mt-1">
                Location: {s.location} · Price: {money(s.price)} · Booked:{" "}
                {booked} / Capacity: {capacity} (Remaining: {remaining})
              </div>
              {/* show hint if disabled */}
              {disabled && hint && (
                <div className="text-xs text-amber-700 mt-1">{hint}</div>
              )}
            </div>
            <div>
              <button
