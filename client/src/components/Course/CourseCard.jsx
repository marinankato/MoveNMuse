  ...
  return (
    <li className="rounded-2xl border p-4 shadow-sm hover:shadow-md transition-all">
      ...
      <div className="mt-2 text-xs text-gray-500 flex flex-wrap gap-2">
        {c.category && <span className="rounded-full border px-2 py-0.5">{c.category}</span>}
        {c.level && <span>{c.level}</span>}
        <span>{money(c.price)}</span>
        <span>Remaining {Number(c.remaining || 0)}</span>
        {c.nextStartTime && <span>Next Start {new Date(c.nextStartTime).toLocaleString()}</span>}
      </div>
      <div className="mt-3 flex items-center gap-2">
        ...
        {c.lowCapacity && <span className="text-xs text-red-600">Limited Spots</span>}
      </div>
    </li>
  );
}
